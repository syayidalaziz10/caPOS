...
<div class="py-14 px-6">
    ...
    <div class="flex justify-center items-center h-screen w-full bg-warna-2 rounded-2xl">
            ...
            <div class="w-full mb-6">
                <div class="flex space-x-7 mb-7">
                    <div class="text-secondary bg-primary w-1/2 h-40 rounded-3xl px-8 py-6">
                        ...
                        @if($dataDetail != [])<h1 class="font-bold mt-6 text-5xl">{{ $banyakMenu->banyak }}</h1>@else<h1 class="text-componen font-bold text-2xl">Tidak ada Pemesanan</p>@endif
                    </div>
                    <div class="text-secondary bg-primary w-1/2 h-40 rounded-3xl px-8 py-6">
                        <p>Jumlah Menu Yang Terjual</p>
                        @if($dataDetail != [])<h1 class="font-bold mt-6 text-5xl">{{ $jumlahMenu->jumlah }}</h1>@else<h1 class="text-componen font-bold text-2xl">Tidak ada Pemesanan</p>@endif
                    </div>
                </div>
                <div class="text-secondary bg-primary w-full h-40 rounded-3xl px-8 py-6">
                    <p>Total Pemesanan Yang dibuat</p>
                    @if($dataDetail != [])<h1 class="font-bold mt-6 text-5xl"> {{$totalPesanan}} </h1>@else<h1 class="text-componen font-bold text-2xl">Tidak ada Pemesanan</p>@endif
                </div>
            </div>
        ...
        <div class="h-full w-2/5 rounded-l-2xl shadow-2xl py-8 px-12">
            ...
            <div>
                <p class="font-light font-sm text-secondary">PENDAPATAN HARI INI</p>
